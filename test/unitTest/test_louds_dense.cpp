...
namespace surf {
    ...
    namespace densetest {
        ...
        static std::vector<uint64_t> values_words;
        ...
        void DenseUnitTest::newBuilder(SuffixType suffix_type, level_t suffix_len) {
            builder_ = new SuRFBuilder(kIncludeDense, kSparseDenseRatio);
        }
        ...
        void DenseUnitTest::testLookupWord() {
            ...
            uint64_t value;
            for (unsigned i = 0; i < words.size(); i++) {
                bool key_exist = louds_dense_->lookupKey(words[i], out_node_num, value);
            ...
            for (unsigned i = 0; i < words.size(); i++) {
                for (unsigned j = 0; j < words_trunc_[i].size() && j < words[i].size(); j++) {
                    ...
                    key[j] = 'A';
                    bool key_exist = louds_dense_->lookupKey(key, out_node_num, value);
        ...
        TEST_F (DenseUnitTest, lookupWordTest) {
            for (int t = 0; t < kNumSuffixType; t++) {
                for (int k = 0; k < kNumSuffixLen; k++) {
                    ...
                    builder_->build(words, values_words);
        ...
        TEST_F (DenseUnitTest, lookupIntTest) {
            newBuilder(kReal, 8);
            builder_->build(ints_, values_);
            louds_dense_ = new LoudsDense(builder_);
            position_t out_node_num = 0;
            uint64_t value;

            for (uint64_t i = 0; i < kIntTestBound; i += kIntTestSkip) {
                bool key_exist = louds_dense_->lookupKey(uint64ToString(i), out_node_num, value);
                if (i % kIntTestSkip == 0) {
                    ASSERT_TRUE(key_exist);
